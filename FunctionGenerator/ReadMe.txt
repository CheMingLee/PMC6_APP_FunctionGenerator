================================================================================
    MFC 程式庫 : FunctionGenerator 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 FunctionGenerator 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 FunctionGenerator 應用程式的檔案。

FunctionGenerator.vcproj
...
FunctionGenerator.h
    ...
    並宣告 CFunctionGeneratorApp 應用程式類別。

FunctionGenerator.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CFunctionGeneratorApp。

FunctionGenerator.rc
    這份清單包含了所有程式要使用的 Microsoft Windows 資源。其中包括了儲存
    在 RES 子目錄下的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 
    內編輯這個檔案。您的專案資源在 1028。

res\FunctionGenerator.ico
    這個圖示檔是用來做為應用程式的圖示。這個圖示包含在主要資源檔
    FunctionGenerator.rc 內。

res\FunctionGenerator.rc2
...
FunctionGeneratorDlg.h、FunctionGeneratorDlg.cpp - 對話方塊
    這些檔案包含您的 CFunctionGeneratorDlg 類別。這個類別定義應用程式主
    對話方塊的行為。對話方塊的範本在 FunctionGenerator.rc 內，您可以 
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 FunctionGenerator.pch 的先行編譯標頭 (PCH) 檔，
